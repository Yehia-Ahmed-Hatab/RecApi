...
class Recommender:
	...
	def recommendProducts(self, user, nums_of_products):
		x_vars = []
		for col in cat_cols:
		...
		x_vars += user['prev_products']
		x_vars = np.array(x_vars)
		x_vars = x_vars.reshape(1, -1)
		xg_test = xgb.DMatrix(x_vars)
		preds = self.model.predict(xg_test)
		preds = np.argsort(preds[0])[::-1]
		preds = preds[:nums_of_products]
		preds = [target_cols[i] for i in preds]
		return preds
